...
if test "$PHP_KRB5" != "no" -o "$PHP_KRB5KADM" != "no"; then
	...
	AC_CHECK_LIB(krb5, krb5_cc_new_unique, 
		...
		,
		[ -L $KERBEROS_DIR/lib/x86_64-linux-gnu/mit-krb5 ] )
	...
	PHP_ADD_LIBRARY_WITH_PATH(krb5, $KERBEROS_DIR/lib/x86_64-linux-gnu/mit-krb5, KRB5_SHARED_LIBADD)
	...
	if test "$IMPLEMENTATION" = "heimdal"; then
		...
		PHP_ADD_LIBRARY_WITH_PATH(gssapi_krb5, $KERBEROS_DIR/lib/x86_64-linux-gnu/mit-krb5, KRB5_SHARED_LIBADD)
	fi

	
	if test "$PHP_KRB5KADM" != "no"; then
		PHP_ADD_LIBRARY_WITH_PATH(kadm5clnt, $KERBEROS_DIR/lib/x86_64-linux-gnu/mit-krb5, KRB5_SHARED_LIBADD)
