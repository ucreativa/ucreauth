...
if test "$PHP_KRB5" != "no" -o "$PHP_KRB5KADM" != "no"; then
	...
	if test "$PHP_KRB5KADM" != "no"; then
		...
		if test -f /usr/include/mit-krb5/kadm5/admin.h ; then
 			AC_MSG_RESULT([yes])
	...
	for DIRECTORY in $PHP_KRB5; do
		test -r "$DIRECTORY/lib/i386-linux-gnu/mit-krb5/libgssapi_krb5.so" && test -r "$DIRECTORY/lib/i386-linux-gnu/mit-krb5/libkrb5.so" && KERBEROS_DIR=$DIRECTORY && break

		test -r "$DIRECTORY/lib64/mit-krb5/libgssapi_krb5.so" && test -r "$DIRECTORY/lib64/mit-krb5/libkrb5.so" && KERBEROS_DIR=$DIRECTORY && break
	done
	...
	if test -z "$KERBEROS_DIR" ; then
		...
		PHP_ADD_INCLUDE(/usr/include/mit-krb5/)
	fi
	...
	AC_CHECK_LIB(krb5, krb5_cc_new_unique, 
		...
		,
		[ -L $KERBEROS_DIR/lib/i386-linux-gnu/mit-krb5 ] )
	...
	PHP_ADD_LIBRARY_WITH_PATH(krb5, $KERBEROS_DIR/lib/i386-linux-gnu/mit-krb5, KRB5_SHARED_LIBADD)
	...
	if test "$IMPLEMENTATION" = "heimdal"; then
		...
		PHP_ADD_LIBRARY_WITH_PATH(gssapi_krb5, $KERBEROS_DIR/lib/i386-linux-gnu/mit-krb5, KRB5_SHARED_LIBADD)
	fi

	
	if test "$PHP_KRB5KADM" != "no"; then
		PHP_ADD_LIBRARY_WITH_PATH(kadm5clnt, $KERBEROS_DIR/lib/i386-linux-gnu/mit-krb5, KRB5_SHARED_LIBADD)
